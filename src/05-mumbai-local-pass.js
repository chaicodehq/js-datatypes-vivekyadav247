...
export function generateLocalPass(passenger) {
  ...
  const passId = classTypeLower.charAt(0).toUpperCase() + from.toUpperCase().slice(0, 3) + to.toUpperCase().slice(0, 3);
  const nameUpper = name.toUpperCase();
  const fromTitleCase = from.charAt(0).toUpperCase() + from.slice(1).toLowerCase();
  const toTitleCase = to.charAt(0).toUpperCase() + to.slice(1).toLowerCase();
